import * as React from 'react';
import { mount } from 'enzyme';
import { expect } from 'chai';
// @ts-ignore
import Avatar from './index';

describe('<Avatar />', () => {
  const imgLink = 'data:image/jpeg;base64,';

  it('render without errors', () => {
    const wrapper = mount(<Avatar />);

    expect(wrapper.find('img').exists()).to.be.true;
  });

  it('render with image', () => {
    const wrapper = mount(<Avatar src={imgLink} />);

    expect(wrapper.find('[data-component="avatar"]').exists()).to.be.true;
    expect(wrapper.find('img').prop('src')).to.equal(imgLink);
  });

  it('change size', () => {
    const wrapper = mount(<Avatar size={80} />);

    expect(wrapper.find('img').prop('style')).to.have.property('height', 80);
    expect(wrapper.find('img').prop('style')).to.have.property('width', 80);
  });

  it('with styles', () => {
    const wrapper = mount(
      <Avatar
        size={80}
        style={{ opacity: 0.7 }}
      />,
    );

    expect(wrapper.find('img').prop('style')).to.have.property('opacity', 0.7);
  });

  it('with letters', () => {
    const wrapper = mount(
      <Avatar
        letters="Admin"
      />,
    );

    expect(wrapper.find('div').text()).to.equal('A');
    expect(wrapper.find('div').prop('className')).to.equal('avatar round_circle truncate_text fill_light_grey stroke_light_grey text_primary');
    expect(wrapper.find('div').prop('style')).to.have.property('lineHeight', '38px');
  });

  it('change theme', () => {
    const wrapper = mount(
      <Avatar
        letters="User"
        bgType="stroke"
        color="secondary"
        textColor="black"
      />,
    );

    expect(wrapper.find('div').text()).to.equal('U');
    expect(wrapper.find('div').prop('className')).to.equal('avatar round_circle truncate_text stroke_secondary text_black');
  });

  it('with className', () => {
    const wrapper = mount(
      <Avatar
        letters="Custom"
        className="my_custom_class"
      />,
    );

    expect(wrapper.find('div')
      .prop('className'))
      .to.equal('avatar round_circle truncate_text fill_light_grey stroke_light_grey text_primary my_custom_class');
  });
});
